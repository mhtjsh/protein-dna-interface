...
Reads all .int files for a given PDB ID from the rsa/ directory,
computes interface metrics, and writes a summary CSV into interface/.
"""
import os
import glob
import argparse
...
pdb_id = args.pdb_id.upper()  # normalize to uppercase
...
int_paths = [
    ...
    if re.match(rf"{pdb_id}[A-Za-z]\.int$", os.path.basename(f))
]
...
for path in int_paths:
    with open(path) as f:
        for line in f:
            ...
            parts = line.split()
            try:
                asa_m = float(parts[-2])  # ASA in monomer
                asa_c = float(parts[-1])  # ASA in complex
...
# ----------------------
# 6. Build and write summary table
# ----------------------
